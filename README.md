# good_boilerplate_cli

Mason workspace for generating Flutter starter projects.

## Setup

```sh
dart pub global activate mason_cli
mason get
```

## Generate a Starter

From this Mason workspace:

```sh
cd /Users/zianfahrudy/Documents/ZIANDEV/good_boilerplate_cli
mason get
mason make good_boilerplate -q
```

Example with custom app metadata via `config.json`:

```json
{
  "project_name": "my_app",
  "app_id": "id.ziandev.my_app",
  "app_name": "My App",
  "description": "A production-ready Flutter starter.",
  "theme_style": "neubrutalism"
}
```

```sh
mason make good_boilerplate -q -c config.json
```

After the brick is published to BrickHub, install it globally once:

```sh
mason add -g good_boilerplate
```

To use the local brick from any directory before publishing, register it globally once:

```sh
mason add -g good_boilerplate --path /Users/zianfahrudy/Documents/ZIANDEV/good_boilerplate_cli/bricks/good_boilerplate
```

Then run from the target output folder:

```sh
cd /Users/zianfahrudy/Documents
mason make good_boilerplate
```

Use `-q` when generating if you want Mason to hide the long per-file `created ...` list and keep the output concise.

The `good_boilerplate` brick provides a ready baseline that can be expanded with native platform flavor settings when bundle ids, schemes, and app icons are finalized.
By default, its post-generation hook runs `flutter create` to add `android`, `ios`, `web`, and `macos` platform folders.
Do not use `--no-hooks` if you want those native platform folders.

## Current Starter Features

- Flutter `>=3.47.0` and Dart SDK `^3.13.0`.
- Dev and prod flavor entry points.
- Native Flutter platform folders generated by `flutter create`: `android`, `ios`, `web`, and `macos`.
- VS Code launch configurations for dev and prod.
- Selectable `soft_pastel` (default) or `neubrutalism` generated theme style.
- Per-flavor config for app id, app name, base API URL, network logging, upgrade checks, Wiredash, and RevenueCat offerings.
- Dio, Talker Dio logger, Flutter Bloc/Cubit, GoRouter, cached network images, GetIt, Upgrader `13.6.0`.
- Json Serializable setup with an example model.
- Three-slide reusable onboarding flow with stepper, headline, subheadline, illustration, button, and trusted info.
- Settings page with reusable rows/cards, persisted dark/light mode, and a Wiredash feedback section.
- Reusable paywall page with RevenueCat plans, dummy fallback plans, delayed close button, free-trial toggle, restore, terms, and privacy links.
- Reusable daily quota service using local storage for per-feature usage limits.
- Reusable RevenueCat service with `default` and `promo_offer` offering identifiers.
- Non-blocking Shorebird code-push update checks using `shorebird_code_push`.
- Reusable AdMob service and banner/interstitial/rewarded helpers.
- Reusable theme colors and typography.
